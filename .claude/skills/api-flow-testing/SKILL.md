...
> An Antigravity-compatible twin of this file lives at
> [.agents/skills/api-flow-testing/SKILL.md](../../../.agents/skills/api-flow-testing/SKILL.md)
> (Google Antigravity discovers skills from `.agents/skills/`, not
> `.claude/skills/`). Both files should stay in sync — if you change one,
> update the other. They share the same test runner script, so only the
> instructions are duplicated, not the logic.
...
The bundled runner is stdlib-only Python (`urllib`) — no `pip install`
needed, works with any `python3`.
...
`flow_*` function in [scripts/run_api_flow_tests.py](scripts/run_api_flow_tests.py)
rather than writing a one-off script:
...
writes both a Markdown and a JSON report into
[reports/](reports/) (or `--out-dir`), plus exits non-zero if anything failed
or errored (skips don't affect the exit code).
